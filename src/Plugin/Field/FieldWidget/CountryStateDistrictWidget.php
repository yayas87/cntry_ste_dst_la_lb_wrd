 ...
 *   label = @Translation("Country state district widget"),
 *   field_types = {
 *     "cntry_ste_dst_la_lb_wrd_type"
 *   }
 * )
 */
class CountryStateDistrictWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    if ($country_id) {
      $element['state'] = [
        ...
        '#ajax' => [
          ...
          'progress' => [
            'type' => 'throbber',
            'message' => $this->t('Searching cities...'),
          ],
        ],
      ];
    }
    ...
    if ($state_id) {
      $element['district'] = [
        ...
        '#empty_option' => $this->t('-- Select an option --'),
        '#required' => $this->fieldDefinition->isRequired(),
        '#title' => !empty($this->getFieldSetting('district_lable')) ? $this->getFieldSetting('district_lable') : $this->t('District'),
        '#validated' => TRUE,
        '#attributes' => [
          'class' => [
